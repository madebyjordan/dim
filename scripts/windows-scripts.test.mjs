...
function fixture() {
  const directory = mkdtempSync(resolve(tmpdir(), "eclipse-windows-scripts-test-"));
  const repository = resolve(directory, "repo");
  const bin = resolve(directory, "bin");
  mkdirSync(resolve(repository, "scripts"), { recursive: true });
  mkdirSync(bin);
  copyFileSync(resolve(root, "scripts/bootstrap.sh"), resolve(repository, "scripts/bootstrap.sh"));
  copyFileSync(resolve(root, "scripts/run.sh"), resolve(repository, "scripts/run.sh"));
  ...
  executable(resolve(bin, "ffmpeg"), "#!/usr/bin/env bash\necho ffmpeg-fixture\n");
  executable(resolve(bin, "ffprobe"), "#!/usr/bin/env bash\necho ffprobe-fixture\n");
...
test("Windows bootstrap copies media executables and preserves existing copies", () => {
  const item = fixture();
  try {
    const first = spawnSync("/bin/bash", ["./scripts/bootstrap.sh", "--skip-ui", "--skip-rust"], {
      cwd: item.repository,
      env: item.env,
      encoding: "utf8",
    });
    ...
    assert.equal(readFileSync(ffmpeg, "utf8"), "#!/usr/bin/env bash\necho ffmpeg-fixture\n");
    assert.equal(readFileSync(ffprobe, "utf8"), "#!/usr/bin/env bash\necho ffprobe-fixture\n");

    writeFileSync(ffmpeg, "preserved ffmpeg\n");
    chmodSync(ffmpeg, 0o755);
    const second = spawnSync("/bin/bash", ["./scripts/bootstrap.sh", "--skip-ui", "--skip-rust"], {
      cwd: item.repository,
      env: item.env,
      encoding: "utf8",
    });
...
`,
    ...
    const result = spawnSync("/bin/bash", ["./scripts/run.sh", "--release", "--bind-address", "127.0.0.1"], {
      cwd: item.repository,
      env: item.env,
      encoding: "utf8",
    });
    assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
    assert.equal(readFileSync(resolve(item.directory, "runtime.cwd"), "utf8").trim(), realpathSync(release));
    assert.equal(
